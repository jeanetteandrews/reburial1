...
function visualizeFrequency(word, count, wordElement) {
    ...
    const centerX = wordRect.left - stageRect.left + wordRect.width / 2;
    const centerY = wordRect.top - stageRect.top + wordRect.height / 2;
    
    // Scale delay based on frequency - higher frequency = faster feed
    let delay;
    if (count <= 50) {
        delay = 30; // Normal speed for low counts
    } else if (count <= 150) {
        delay = 15; // Faster for medium counts
    } else {
        delay = 5; // Very fast for high counts
    }
    ...
    for (let i = 0; i < count; i++) {
        setTimeout(() => {
            ...
            // Random orbit radius
            const radius = 30 + Math.random() * 40; // Between 30px and 70px
            
            // Random starting angle instead of evenly distributed
            const startAngle = Math.random() * 360; // Random angle between 0-360
